...
const QuizResults: React.FC<QuizResultsProps> = ({
  ...
  return (
    <div className="flex flex-col py-8 px-4 2xl:py-10 2xl:px-6 space-y-8 2xl:space-y-10">
      {/* Title */}
      <div className="text-center space-y-2">
        <div className="rounded-full bg-primary/10 p-4 w-fit mx-auto">
          <Trophy className="h-10 w-10 2xl:h-12 2xl:w-12 text-primary" />
        </div>
        <h3 className="text-xl 2xl:text-2xl font-semibold">{t('resultComplete')}</h3>
      </div>

      {/* Stats Row */}
      <div className="grid grid-cols-3 gap-3 2xl:gap-4">
        <div className="rounded-xl border p-4 2xl:p-5 text-center space-y-1">
          <p className="text-xs 2xl:text-sm text-muted-foreground font-medium">{t('statsScore')}</p>
          {analysisLoading ? (
            <Shimmer className="h-8 w-16 mx-auto" />
          ) : (
            <p className="text-2xl 2xl:text-3xl font-bold text-primary">{score}/{total}</p>
          )}
        </div>
        <div className="rounded-xl border p-4 2xl:p-5 text-center space-y-1">
          <p className="text-xs 2xl:text-sm text-muted-foreground font-medium">{t('statsAccuracy')}</p>
          {analysisLoading ? (
            <Shimmer className="h-8 w-14 mx-auto" />
          ) : (
            <p className="text-2xl 2xl:text-3xl font-bold text-primary">{percentage}%</p>
          )}
        </div>
        <div className="rounded-xl border p-4 2xl:p-5 text-center space-y-1">
          <p className="text-xs 2xl:text-sm text-muted-foreground font-medium">{t('statsBreakdown')}</p>
          {analysisLoading ? (
      ...
      {analysisLoading ? (
        <div className="space-y-3">
          <h4 className="text-sm 2xl:text-base font-semibold">{t('highlightsTitle')}</h4>
        ...
        <div className="space-y-2">
          <h4 className="text-sm 2xl:text-base font-semibold">{t('highlightsTitle')}</h4>
          <ul className="text-sm 2xl:text-base text-muted-foreground space-y-1 pl-6 list-disc">
      ...
      {analysisLoading ? (
        <div className="space-y-3">
          <h4 className="text-sm 2xl:text-base font-semibold">{t('focusAreasTitle')}</h4>
          <div className="space-y-2 pl-6">
